(function ($) {
	...
	function variartion_images_init() {
		$( '#variable_product_options' ).on( 'click', '.motta-variation-images-upload', function( event ) {
			var $el = $( this ),
				$image_gallery_ids = $el.closest('.motta-variation-images-container').find( '.motta_variation_images' ),
				$product_images    = $el.closest('.motta-variation-images-container').find( 'ul.variation-images-list' );
		...
		$( '#variable_product_options' ).find('ul.variation-images-list').sortable({
			...
			update: function() {
				...
				$(this).closest('.motta-variation-images-container').find( 'ul.variation-images-list li.image' ).css( 'cursor', 'default' ).each( function() {
					var attachment_id = $( this ).attr( 'data-attachment_id' );
					attachment_ids = attachment_ids + attachment_id + ',';
				});

				$(this).closest('.motta-variation-images-container').find( '.motta_variation_images' ).val( attachment_ids );
	...
	function variartion_images_position() {
		$('.woocommerce_variation').each(function () {
			...
			var galleryWrapper = $(this).find('.motta-variation-images-container');
