jQuery(document).ready(function ($) {
	...
	var $tab_default = $('.wp-list-table .motta_product_tab_type-default');
	$tab_default.find('.check-column input[type="checkbox"]').remove();

	$( '#the-list' ).find('.motta_product_tab_type-default').each(function() {
	...
	var $tab_settings = $('#motta-product-tabs-settings'),
		$tab_type = $tab_settings.find('.motta-product-tab--input'),
		$tab_product = $tab_settings.find('.motta-product-tabs--product'),
		$tab_categories = $tab_settings.find('.motta-product-tabs--categories');
	...
	$( 'table.widefat tbody' ).sortable({
		...
		update: function( event, ui ) {
			...
			$.post(
				ajaxurl,
				{ action: 'motta_product_tab_ordering', id: postid, previd: prevpostid, nextid: nextpostid },
				function( response ) {
