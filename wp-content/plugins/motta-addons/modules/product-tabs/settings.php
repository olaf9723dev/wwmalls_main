...
class Settings  {
	...
	const OPTION_NAME   = 'motta_product_tab';
	...
	public function __construct() {
		...
		add_action( 'motta_after_product_tab_ordering', array( $this, 'clear_product_tabs_cache' ) );
		...
		add_action( 'wp_ajax_motta_product_tab_ordering', array( $this, 'product_tab_ordering' ) );
	...
	public function product_tabs_section( $sections ) {
		$sections['motta_addons_product_tabs'] = esc_html__( 'Product Tabs', 'motta-addons' );
	...
	public function product_tabs_settings( $settings, $section ) {
		if ( 'motta_addons_product_tabs' != $section ) {
			return $settings;
		}
		...
		$settings_product_tabs = array(
			...
			array(
				'name'    => esc_html__( 'Product Tabs', 'motta-addons' ),
				'desc'    => esc_html__( 'Enable product tabs manager', 'motta-addons' ),
				'id'      => self::OPTION_NAME,
				'default' => 'no',
				'type'    => 'checkbox',
				'checkboxgroup' => 'start',
	...
	public function edit_admin_columns( $columns ) {
		$columns = array_merge( $columns, array(
			'product_tab_disable' => esc_html__( 'Disabled', 'motta-addons' ),
			'product_tab_type' => esc_html__( 'Type', 'motta-addons' ),
			'product_tab_products' => esc_html__( 'Products', 'motta-addons' ),
			'product_tab_categories' => esc_html__( 'Categories', 'motta-addons' ),
		) );
	...
	public function manage_custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'product_tab_disable':
				...
				echo $tab_disable == 'yes' ? esc_html__( 'Yes', 'motta-addons' ) : esc_html__( 'No', 'motta-addons' );
			...
			case 'product_tab_type':
				...
				if ( ! is_wp_error( $types ) && $types && is_array( $types ) ) {
					...
					switch( $type_name ) {
						case 'global':
							esc_html_e('Global', 'motta-addons') ;
							break;
						case 'product':
							esc_html_e('Product', 'motta-addons') ;
							break;
						case 'custom':
							esc_html_e('Custom', 'motta-addons') ;
							break;
						default:
							esc_html_e('Default', 'motta-addons') ;
							break;
					}
				} else {
					esc_html_e( 'No Type', 'motta-addons' );
				}
			...
			case 'product_tab_products':
				...
				if (  $product_ids ) {
					...
					$links[] = esc_html__( 'No Product', 'motta-addons' );
				}
			...
			case 'product_tab_categories':
				...
				if ( ! is_wp_error( $cats ) && $cats && is_array( $cats ) ) {
					...
					$links[] = esc_html__( 'No Category', 'motta-addons' );
				}
	...
	public function meta_boxes( $post ) {
		add_meta_box( 'motta-product-tabs', esc_html__( 'Tabs Settings', 'motta-addons' ), array( $this, 'tabs_meta_box' ), self::POST_TYPE, 'advanced', 'high' );
	}
	...
	public function tabs_meta_box( $post ) {
		?>
		<div id="motta-product-tabs-settings" class="motta-product-tabs-settings">
			<p class="form-field">
				<label><?php esc_html_e('Tab Type', 'motta-addons'); ?></label>
				<span class="motta-product-tabs--type">
					<?php
					$terms = get_the_terms( $post->ID, self::TAXONOMY_TAB_TYPE );
					$tab_type = ! is_wp_error( $terms ) && $terms ? $terms[0]->name : 'global';
					?>
					<input type="radio" id="motta-product-tab-global" <?php echo $tab_type == 'global' ? 'checked' : ''; ?> class="motta-product-tab--input" checked name="_product_tab_type" value="global">
					<label for="motta-product-tab-global"><?php esc_html_e('Global', 'motta-addons'); ?></label>
					<input type="radio" id="motta-product-tab-product" <?php echo $tab_type == 'product' ? 'checked' : ''; ?> class="motta-product-tab--input" name="_product_tab_type" value="product">
					<label for="motta-product-tab-product"><?php esc_html_e('Product', 'motta-addons'); ?></label>
					<input type="radio" id="motta-product-tab-custom" <?php echo $tab_type == 'custom' ? 'checked' : ''; ?> class="motta-product-tab--input" name="_product_tab_type" value="custom">
					<label for="motta-product-tab-custom"><?php esc_html_e('Custom', 'motta-addons'); ?></label>
				</span>

			</p>
			<p class="form-field motta-product-tabs--product">
				<label for="product-tab-id"><?php esc_html_e( 'Products', 'motta-addons' ); ?></label>
				<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="product-tab-id" name="_product_tab_product_ids[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'motta-addons' ); ?>" data-action="woocommerce_json_search_products">
					<?php
			...
			<p class="form-field motta-product-tabs--categories">
				<label for="product_tab_categories"><?php esc_html_e( 'Categories', 'motta-addons' ); ?></label>
				<select class="wc-category-search" multiple="multiple" style="width: 50%;" id="product_tab_categories" name="_product_tab_cat_slugs[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a category&hellip;', 'motta-addons' ); ?>" data-action="woocommerce_json_search_categories">
					<?php
	...
	public function create_post_type() {
		$posts = array(
			'description' 				=>	esc_html__('Description', 'motta-addons'),
			'additional_information' 	=>	esc_html__('Additional Information', 'motta-addons'),
			'reviews' 					=>	esc_html__('Reviews', 'motta-addons'),
		);
	...
	public static function edit_custom_box($column_name, $post_type) {
		...
		<fieldset class="inline-edit-col-left">
			<div class="inline-edit-col">
				<label>
					<span class="title"><?php esc_html_e('Disable', 'motta-addons'); ?></span>
	...
	public static function product_tab_ordering() {
		...
		$menu_orders = wp_list_pluck( $wpdb->get_results( "SELECT ID, menu_order FROM {$wpdb->posts} WHERE post_type = 'motta_product_tab' ORDER BY menu_order DESC" ), 'menu_order', 'ID' );
		...
		foreach ( $menu_orders as $id => $menu_order ) {
			...
			*/
			do_action( 'motta_after_single_product_tab_ordering', $id, $index );
		}
		...
		do_action( 'motta_after_product_tab_ordering', $sorting_id, $menu_orders );
	...
	function product_tab_redirect() {
		global $pagenow;
		if($pagenow == 'post.php' && isset($_GET['post']) && get_post_type( $_GET['post'] ) == 'motta_product_tab'){
			$terms = get_the_terms( $_GET['post'], self::TAXONOMY_TAB_TYPE );
			$tab_type = ! is_wp_error( $terms ) && $terms ? $terms[0]->name : '';
			if( $tab_type == 'default' ) {
				wp_redirect(admin_url('/edit.php?post_type=motta_product_tab' ));
				exit;
			}
	...
	public function clear_product_tabs_cache() {
		delete_transient( 'motta_wc_product_tabs' );
	}
	...
	public function admin_scripts( $hook ) {
		...
		if ( in_array( $hook, array('edit.php', 'post-new.php', 'post.php' ) ) && self::POST_TYPE == $screen->post_type ) {
			wp_enqueue_style( 'motta-product-tabs', MOTTA_ADDONS_URL . 'modules/product-tabs/assets/product-tabs-admin.css' );
			wp_enqueue_script( 'motta-product-tabs', MOTTA_ADDONS_URL . 'modules/product-tabs/assets/product-tabs-admin.js', array( 'jquery', 'jquery-ui-sortable' ),'1.0', true );
