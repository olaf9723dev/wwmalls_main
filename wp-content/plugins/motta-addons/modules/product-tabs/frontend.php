...
class FrontEnd {
	...
	public function __construct() {
		...
		//Add count Reviews
		add_filter( 'woocommerce_product_tabs', array( $this, 'rename_reviews_tab' ), 21);
	}
	 ...
	 * @return void
	 */
	public function rename_reviews_tab( $tabs ) {
		global $product;

		if ( ! empty( $tabs['reviews'] ) ) {
			$review_count = $product->get_review_count();
		if ( $review_count > 0 ) {
				$tabs['reviews']['title'] .= esc_html( ' (' . $review_count . ') ' );
			}
		}
			...
			switch( $tab_type ) {
				case 'global':
					if( ! isset( $tabs[$tab->post_name] ) && $this->is_global_tab($tab_id) && ! empty($tab->post_content) ) {
						...
						$tabs[$tab->post_name]['callback'] = 'Motta\Addons\Modules\Product_Tabs\FrontEnd::get_product_tab_content';
					}
					break;
				case 'product':
				...
				case 'custom':
					...
					if( ! isset( $tabs[$tab->post_name] ) && $this->is_custom_tab($tab_id, $has_content) ) {
						...
						$tabs[$tab->post_name]['priority'] = $index;
						$tabs[$tab->post_name]['tab_id'] = $tab_id;
						$tabs[$tab->post_name]['callback'] = 'Motta\Addons\Modules\Product_Tabs\FrontEnd::get_product_custom_tab_content';
					}
					break;
				case 'default':
					if( isset( $tabs[$tab->post_name] ) ) {
	...
	public function get_product_tabs() {
		$transient_name = 'motta_wc_product_tabs';
