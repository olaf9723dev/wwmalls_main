...
class Frontend {
	...
	public function scripts() {
		wp_enqueue_style( 'motta-size-guide-content', MOTTA_ADDONS_URL . 'modules/size-guide/assets/css/size-guide.css', array(), '1.0' );
		wp_enqueue_script('motta-size-guide-content', MOTTA_ADDONS_URL . 'modules/size-guide/assets/js/size-guide-tab.js');
	}
	...
	public function display_size_guide() {
		...
		$guide_settings = get_post_meta( $product->get_id(), 'motta_size_guide', true );
	...
	public function size_guide_tab( $tabs ) {
		...
		$button_text = $text ? $text : esc_html__('Size Guide' , 'motta-addons');
	...
	protected function get_size_guide_button() {
		...
		$button_text = esc_html__('Size Guide' , 'motta-addons');
		...
		return apply_filters(
			'motta_size_guide_button',
			sprintf(
				'<p class="product-size-guide %s"><a href="#" data-toggle="off-canvas" data-target="size-guide-panel" class="size-guide-button">%s</a></p>',
				$class, $button_text
			)
		);
	}
	...
	public function size_guide_panel() {
		...
		$display = ( is_array( $guide_settings ) && ! empty( $guide_settings['display'] ) ) ? $guide_settings['display'] : $this->get_option( 'display' );
		?>
		<div id="size-guide-panel" class="offscreen-panel size-guide-panel">
			<div class="panel__backdrop"></div>
			<div class="panel__container modal-content">
				<div class="modal-header">
                    <h3 class="title"><?php esc_html_e( 'Size Guide', 'motta-addons' ) ?></h3>
                   <span class="button-close"><?php echo \Motta\Addons\Helper::get_svg( 'close', 'ui' , [ 'class' => 'panel__button-close' ] ) ?></span>
                </div>
				<div class="modal-size-chart motta-scrollbar">
	...
	public function size_guide_content() {
		...
		echo '<div class="motta-size-guide">';

		if ( ! empty( $guide->post_content ) ) {
			echo '<div class="motta-size-guide--global-content">' . apply_filters( 'the_content', $guide->post_content ) . '</div>';
		}
		...
		if ( 1 < count( $size_guides['tables'] ) ) {
			...
			echo '<div class="motta-size-guide-tabs">';
			echo '<ul class="motta-size-guide-tabs__nav">' . implode( '', $tabs ) . '</ul>';
			echo '<div class="motta-size-guide-tabs__panels">';
		}

		foreach ( $size_guides['tables'] as $index => $table ) {
			echo '<div class="motta-size-guide-tabs__panel ' . ( $index ? '' : 'active' ) . '" data-panel="' . esc_attr( $index + 1 ) . '">';

			if ( ! empty( $size_guides['names'][ $index ] ) ) {
				echo '<h4 class="motta-size-guide__name">' . wp_kses_post( $size_guides['names'][ $index ] ) . '</h4>';
			}

			if ( ! empty( $size_guides['descriptions'][ $index ] ) ) {
				echo '<div class="motta-size-guide__description">' . wp_kses_post( $size_guides['descriptions'][ $index ] ) . '</div>';
			}

			if ( ! empty( $table ) ) {
				$table = json_decode( $table, true );

				echo '<table class="motta-size-guide__table">';
			...
			if ( ! empty( $size_guides['information'][ $index ] ) ) {
				echo '<div class="motta-size-guide__info">' . wp_kses_post( $size_guides['information'][ $index ] ) . '</div>';
			}
	...
	public function get_product_size_guide_id( $object = false ) {
		...
		$size_guide = get_post_meta( $_product->get_id(), 'motta_size_guide', true );
