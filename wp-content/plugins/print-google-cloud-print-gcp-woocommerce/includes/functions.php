...
function get_shipping_details($order)
{
		...
		$shipping_label = class_exists('\ZZHoursDelivery\Settings') ?
			\ZZHoursDelivery\Settings::get_delivery_label($order->get_meta('_zh_shipping_type')) :
			\ZOrder_Manager\Setting\Delivery::get_delivery_label($order->get_meta('_zh_shipping_type'));
		$date = date_i18n(\get_option('date_format', 'm/d/Y'), strtotime($order->get_meta('_zh_shipping_date')));
		$time = function_exists('\ZZHoursDelivery\cast_to_time_format') ?
			\ZZHoursDelivery\cast_to_time_format($order->get_meta('_zh_shipping_time')) :
			\ZOrder_Manager\cast_to_time_format($order->get_meta('_zh_shipping_time'));
