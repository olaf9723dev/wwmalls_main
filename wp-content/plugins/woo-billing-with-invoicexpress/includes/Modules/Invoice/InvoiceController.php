...
/* JSON API ready */
/* WooCommerce HPOS ready 2023-07-12 */
...
class InvoiceController extends BaseController {
	...
	public function register_hooks() {
		...
		add_action( 'after_setup_theme', function() {
			...
			// AJAX request to load custom forms - Not needed
	...
	//		$result = $order_object->get_created_via() === 'checkout'; //Error 500 (since ever because $order_object does not exist)
	...
	public function order_actions( $actions, $order_object = null ) {
		...
		$generate_invoice = apply_filters( 'invoicexpress_woocommerce_order_action_title', $generate_invoice, $order_object, $this->document_type, 'hd_wc_ie_plus_generate_invoice' );
	...
	public function doAction( $order_object, $mode = 'manual' ) {
		...
		if (
			...
			$order_object->update_meta_data( 'hd_wc_ie_plus_'.$this->document_type.'_permalink', $return['object']->invoice->permalink );
			$order_object->update_meta_data( 'hd_wc_ie_plus_'.$this->document_type.'_type', $this->document_type );
			$order_object->save();

			do_action( 'invoicexpress_woocommerce_after_document_issue', $order_object->get_id(), $this->document_type );

			do_action( 'invoicexpress_woocommerce_debug', 'Invoice issued', $order_object, array(
