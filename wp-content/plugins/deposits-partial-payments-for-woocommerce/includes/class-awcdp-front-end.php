...
class AWCDP_Front_End
{
    ...
    function awcdp_deposits_form($product_id, $price = false){
      ...
      $suffix = apply_filters('awcdp_per_item_deposit_suffix', $suffix);
  ...
  function awcdp_get_deposit_amount($product_id){
    ...
    function awcdp_display_item_subtotal( $output, $cart_item, $cart_item_key ) {
        ...
        if (isset($cart_item['awcdp_deposit'], $cart_item['awcdp_deposit']['enable']) && $cart_item['awcdp_deposit']['enable'] == 1 && isset($cart_item['awcdp_deposit']['deposit']) ) {
          ...
          $payable_in_deposit_text = esc_html__( 'payable in deposit', 'deposits-partial-payments-for-woocommerce' );
          $payable_in_deposit_text = apply_filters('awcdp_payable_in_deposit_text', $payable_in_deposit_text);

          if(apply_filters('awcdp_hide_payable_text_if_depsoit_zero',true, $display_deposit )){

            //$output = wc_price($display_deposit);
            $output .= '<br/><small>( ' . wp_kses_post( sprintf( esc_html__( '%s %s' ), wc_price( $display_deposit ), $payable_in_deposit_text )) . ' )</small>';
        ...
        try {
          ...
          if (( $payment_type == 'deposit' && !$parent->needs_payment() ) ||  ($payment_type != 'deposit' && (!$parent->needs_payment() || $parent->get_status() != 'partially-paid'))) {
            if (did_action('before_woocommerce_pay') && !did_action('after_woocommerce_pay')) {
              $needs_payment = false;
              wc_print_notice( sprintf( __( 'Main order&rsquo;s status is &ldquo;%s&rdquo;&mdash;it cannot be paid for.', 'deposits-partial-payments-for-woocommerce' ), wc_get_order_status_name( $parent->get_status() ) ) ,'notice');
            }
          }
        } catch (\Exception $e) {
