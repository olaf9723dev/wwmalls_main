    ...
    public static function validate_item_totals( $item_totals, $request ) {
        ...
        foreach ( $item_totals as $item_id => $item_total ) {
            ...
            if ( $item_total > $order_line_item_total ) {
                // translators: %s: line item name
                return new WP_Error( 'dokan_pro_refund_error_item_totals', sprintf( __( 'Line item total must not exceed %1$s for item %1$s', 'dokan' ), $order_line_item_total, $order_line_item->get_name() ) );
            }
        }
