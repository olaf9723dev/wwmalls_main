...
class LDDFW_Orders
{
    ...
    public function lddfw_orders_query( $driver_id, $status, $screen = null )
    {
        ...
        if ( 'delivered' === $screen ) {
            ...
            if ( lddfw_is_hpos_enabled() ) {
                // Query for HPOS-enabled environments.
                $result = $wpdb->get_results( $wpdb->prepare( 'SELECT o.ID FROM ' . $wpdb->prefix . 'wc_orders o 
						INNER JOIN ' . $wpdb->prefix . 'lddfw_orders lo ON o.id = lo.order_id
						WHERE o.type = \'shop_order\'
						AND o.status = %s
						AND lo.driver_id = %d
                ...
                // Original query for non-HPOS environments.
                $result = $wpdb->get_results( $wpdb->prepare( 'SELECT p.ID FROM ' . $wpdb->prefix . 'posts p INNER JOIN ' . $wpdb->prefix . 'lddfw_orders o
					ON p.ID = o.order_id
					WHERE
					p.post_type = \'shop_order\'
					AND p.post_status = %s
