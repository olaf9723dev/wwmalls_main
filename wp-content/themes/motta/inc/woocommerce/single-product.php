...
class Single_Product {
	...
	public function open_gallery_summary_wrapper() {
		$class = Helper::get_option( 'product_image_zoom' ) ? 'product-image-zoom' : '';
		echo '<div class="product-gallery-summary ' . $class . '">';
	}
	...
	public function format_sale_price( $price, $regular_price, $sale_price ) {
		...
		if( $product->get_id() == $wp_query->post->ID ) {
	...
	public function share_icons() {
		...
		$args = array();
		$socials = (array) Helper::get_option( 'product_sharing_socials' );
		if ( ( ! empty( $socials ) ) ) {
			$output = array();

			foreach ( $socials as $social => $value ) {
				if( $value == 'whatsapp' ) {
					$args['whatsapp_number'] = Helper::get_option( 'product_sharing_whatsapp_number' );
				}
			...
			echo sprintf( '<div class="motta-social__button-share">%s</div>', implode( '', $output )	);
		};
	}
	 ...
	 * @return void
	 */
	public function modal_socials() {
		if ( ! class_exists( '\Motta\Addons\Helper' ) ||! method_exists( '\Motta\Addons\Helper','share_link' ) || ! Helper::get_option( 'product_sharing' ) ) {
			return;
		}
	 ...
	 * Show Sale
	 */
	public static function get_sale() {
		global $product;
