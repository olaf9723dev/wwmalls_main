...
const gridOptions = {
    rowData: [ ],
    columnDefs: [
        { field: "id", checkboxSelection: true, headerCheckboxSelection: true, headerCheckboxSelectionFilteredOnly: true, maxWidth:150},
        { field: "name", minWidth: 400,},
        { field: "sku" },
        { headerName: "Sale Price", field: "sale_price", filter: "agNumberColumnFilter" },
        { headerName: "Regular Price", field: "regular_price", filter: "agNumberColumnFilter" },
...
function loadData($, data){
    ...
    $.ajax({
        ...
        type: 'post',
        data: {
            action: 'handle_get_products_by_cat',
...
jQuery(document).ready(function($){
    ...
    $('#sites-selector-top').on('change', function() {
        ...
        $.ajax({
            ...
            type: 'post',
            data: {
                action: 'handle_get_category_from_site',
    ...
    // function get_sub_categories(parent_category,categories){
    //     const sub_category_datas = []
    //     if(categories.some(category => category.parent_id === parent_category.id)){
    //         const subcategories = categories.filter(category => category.parent_id === parent_category.id);
    //         subcategories.forEach((subcategory) => {
                
    //             const subcategory_data = {};
    //             const sub_category_items = get_sub_categories(subcategory, categories);
                
    //             subcategory_data.id = subcategory.id;
    //             subcategory_data.text = subcategory.name;
    //             subcategory_data.items = sub_category_items;
    //             sub_category_datas.push(subcategory_data);
    //         });
    //     }
    //     return sub_category_datas;
    // }
    
    // function build_tree_view_select(categories,$){
    //     const category_datas=[]
    //     categories.forEach((category) => {
    //         if(category.parent_id==0 && category.role == "node"){
    //             const category_data = {};
    //             const sub_category_data = get_sub_categories(category, categories);
    //             category_data.id = category.id;
    //             category_data.text = category.name;
    //             category_data.items = sub_category_data;
    //             category_datas.push(category_data);
    //         }
    //     });
    //     const treeView = $('#treeview').dxTreeView({
    //         items: category_datas,
    //         width: 350,
    //         searchEnabled: true,
    //     }).dxTreeView('instance');
    //     init_style($);
...
jQuery(document).ready(function($){
    ...
    if (pageUrl.includes("e-shop")){
        $('.page-header__content').hide();
        $('.dokan-geolocation-locations-map-top').hide();
    }
    if (pageUrl.includes("s=") && pageUrl.includes("post_type=")){
        $('.dokan-geolocation-locations-map-top').hide();
        $('.page-header__content').hide();
    }
