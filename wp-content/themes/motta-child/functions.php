 ...
 * @package Motta Child
 */
require_once 'custom_api/dokan_custom_api.php';
...
function motta_child_enqueue_scripts() {
	...
	wp_enqueue_style( 'custom-styles', get_theme_file_uri('/custom_assets/css/custom.css'));
	wp_enqueue_script( 'custom-scripts', get_theme_file_uri('/custom_assets/js/custom.js'));
	...
	wp_localize_script('custom-scripts', 'ajax_products', array('ajax_url' => admin_url('admin-ajax.php')));
}
add_action( 'admin_enqueue_scripts', 'admin_page_enqueue_scripts' );
function admin_page_enqueue_scripts(){
    wp_enqueue_style( 'custom-styles', get_theme_file_uri('custom_assets/css/admin.css'));
}
...
add_action('admin_menu', 'dokan_vendor_custom_menu', 3000);
...
function dokan_load_template( $query_vars ) {
    ...
    if ( isset( $query_vars['advertisement'] ) ) {
        require_once dirname( __FILE__ ). '/custom_dokan/dashboard/ads.php';
      }
}
